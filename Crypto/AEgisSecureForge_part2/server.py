...
class AEgisSecureForgeProtocol(asyncio.Protocol):
    ...
    def data_received(self, data: bytes):
        ...
        if not datas:
            #logger.debug(f"<|({self.peername}) PACKET Received, no datas")
            return
        ...
        if self._current_cmd:
            ...
            logger.debug(f"<|({self.peername}) {len(self._buffer)} {self._max_buffer_size}")
            if len(self._buffer) == self._max_buffer_size:
...
async def main(port: int = 2429):
    ...
    server = await loop.create_server(
        lambda: AEgisSecureForgeProtocol(), "127.0.0.1", port
    )
    async with server:
        logger.info(
            f"""
